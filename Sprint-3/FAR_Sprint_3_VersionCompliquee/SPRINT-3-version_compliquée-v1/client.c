...
void *envoi_fichier(char *file){
    ...
    int file_size=0;
    int packet_index=0;
    int read_size,envoi,peerSocket;
    ...
    while(1){
        ...
        printf("*** Client connecté, port du client : %d\n",(int) ntohs(cli.sin_port));  
...
void *reception_fichier(struct peer_info *peer_socket){
    ...
    while(recv_size < size_file){
        ...
        if (read_size != write_size){
            printf("erreur les donnéees reçues n'ont pas été enregistrées correctement\n");
        }
        ...
        if(rcv == -1){
            ...
            if (rcv != 1) {
                ...
                if (strncmp("1",buffer,1)==0){
                    ...
                    printf("*** Début de réception de fichier\n");
                    pthread_t recvfile;
